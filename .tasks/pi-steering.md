...
- [ ] Define strict `pi-steering` settings and package defaults.
  ...
  - [ ] Update `pi-steering/package.json` and its lock file to package `AGENTS.yml` and `agents.ts`, use the pinned shared `pi-agents-yaml` package for YAML source handling, and add only the exact compatible TypeBox peer and development dependencies that the schema needs.
...
- [ ] Remove duplicate prompt instructions from the correct source.
  - [ ] Remove the matching acknowledgement and fixed ten-tool reminder rules from the active agent instruction source only after the extension defaults are validated.
  - [ ] Do not edit `./AGENTS.md` for this removal because its current repository content does not contain those duplicate rules; keep all substantive user-instruction, scope, and coding-convention rules unchanged.
