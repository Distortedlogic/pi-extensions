...
- [ ] Apply token-safe defaults and per-node presets only to `pi-preload`
  ...
  - [ ] Set preload defaults to exclude `.git`, `.git/**`, `**/AGENTS.yml`, `.tasks`, `.tasks/**`, `PRELOAD.md`, `TREE.txt`, and the existing dependency lock-file patterns, keeping agent configuration and future task plans outside model-selected preload content.
...
- [ ] Make `pi-tree` consume only `pi-tree` and produce one safe combined tree
  ...
  - [ ] Set tree defaults to include `**/*` and unconditionally exclude `.git`, `.git/**`, `**/AGENTS.yml`, `.tasks`, `.tasks/**`, `.pi/readcache/**`, `.pi/tmp/**`, `PRELOAD.md`, and `TREE.txt`, preventing configuration feedback and future-task lookahead while retaining other lock files, tests, manifests, and tracked configuration names.
  ...
  - [ ] Update existing `pi-tree` tests for absent, empty, explicit-empty, narrowed, excluded, parent-ignored extended, external-prefix, prefix-collision, duplicate-path, ordering, symlink, cycle, and global-limit behavior, including explicit proof that no root or nested `.tasks` or AGENTS.yml path can enter the tree.
...
- [ ] Update generated configuration resources and the extension template
  ...
  - [ ] Update the existing AGENTS.yml authoring skill with missing-section defaults, include replacement, exclude precedence, local Git-ignore behavior, the AGENTS.yml feedback boundary, the `.tasks` future-work boundary, and the external tree-prefix rule.
