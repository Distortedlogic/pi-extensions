...
- [ ] Implement the in-process fold engine module
  ...
  - [ ] Set `GRIT_GLOBAL_DIR` to a stable extension-owned cache path under the Pi agent directory before the first `QueryBuilder` use, so Grit populates and reuses its own module directory there instead of `find_global_grit_dir()` deriving it from the node binary location, which differs per Node installation.
  - [ ] Let Grit perform its normal one-time standard-library fetch into that directory, and do not fabricate a stub module; the `Preserve` clone strategy skips the fetch on every later run.
  - [ ] Resolve that one-time fetch outside the `session_start` deadline, and when it cannot complete, fail the signature-mode path with an error naming the cache path and the network requirement rather than silently emitting unfolded content.
