...
- [ ] Implement the in-process fold engine module
  ...
  - [ ] Record the stub as a known workaround in `src/fold.ts`, not a design: it depends on `clone_repo` short-circuiting on bare directory existence, and the supported alternative is unreachable because `apply_internal` hardcodes `prep_queries(query, vec![])`, so `applyToFile` can never reach the local-`.grit` branch in `init_config_from_path` that skips global module resolution.
  - [ ] Open an upstream issue on `biomejs/gritql` asking that `applyToFile` skip standard-library resolution for a pattern with no imports, and link it from that comment so the workaround can be deleted when it lands.
