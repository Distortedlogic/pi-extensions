...
- [ ] Implement the in-process fold engine module
  ...
  - [ ] Ship a stub global Grit directory in the package holding the empty path `.gritmodules/github.com/getgrit/stdlib`, and set `GRIT_GLOBAL_DIR` to it before the first `QueryBuilder` use, because `find_global_grit_dir()` otherwise resolves to `<node binary>/../.grit` and attempts a standard-library clone, while the `Preserve` clone strategy skips that fetch whenever the path merely exists.
