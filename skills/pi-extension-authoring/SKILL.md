...
Use `pi.extensions.pi-context-preload` in the generated `AGENTS.yml` as the baseline. It preloads runtime TypeScript, package metadata, and the common Pi extension references. It intentionally does not preload `README.md`, test TypeScript files, or lock files.
